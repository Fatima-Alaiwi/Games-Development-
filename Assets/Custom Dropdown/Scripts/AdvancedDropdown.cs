...
public class AdvancedDropdown : MonoBehaviour{
    ...
    // Call this from Start() or from a dedicated Settings Manager
public void InitResolutionDropdown()
{
    // 1. Clear any placeholder options
    DeleteAllOptions();

    // 2. Get system resolutions
    ...
    for (int i = 0; i < resolutions.Length; i++)
    {
        ...
        AddOptions(optionText);

        // Track current resolution to set the default value
    ...
    // 3. Set the current value and update the text
    SelectOption(currentResolutionIndex);

    // 4. Subscribe to the change event
    onChangedValue = (int index) => {
        Resolution res = resolutions[index];
        Screen.SetResolution(res.width, res.height, Screen.fullScreen);
        Debug.Log("Changed Resolution to: " + res.width + "x" + res.height);
    };
}
}
